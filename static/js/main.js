...
function validateRegistrationForm() {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        if (!nombre.value.trim()) {
            showApiMessage('Por favor ingresa tu nombre completo', 'error');
            nombre.focus();
            return false;
        }
        ...
        if (!empresa.value.trim()) {
            showApiMessage('Por favor ingresa el nombre de tu empresa', 'error');
            empresa.focus();
            return false;
        }
        ...
        if (!email.value.trim() || !email.validity.valid) {
            showApiMessage('Por favor ingresa un email válido', 'error');
            email.focus();
            return false;
        }

        // Validar teléfono
        if (!telefono.value.trim() || !validatePhoneNumber(telefono.value)) {
            showApiMessage('Por favor ingresa un número de teléfono válido', 'error');
            telefono.focus();
            return false;
        }
        ...
        try {
            ...
            // Reemplazar el formulario con la respuesta
            const modalContent = this.closest('.modal');
            if (modalContent) {
                modalContent.innerHTML = result;

                // Si es un error, restaurar el botón
                if (tempDiv.querySelector('.alert-error, .alert-warning')) {
            ...
            console.error('Error:', error);
            showApiMessage('Error al procesar el registro. Por favor intenta nuevamente.', 'error');
            submitBtn.disabled = false;
            submitBtn.classList.remove('loading');
            submitBtn.textContent = 'Registrarse';
        }
...
// Modificar el observador de modales para inicializar el formulario de registro
function setupModalObserver() {
    const observer = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
            mutation.addedNodes.forEach(function(node) {
                if (node.nodeType === 1) { // Nodo de elemento
                    // Inicializar el formulario de registro si existe
                    if (node.querySelector('#modal-registro')) {
                        validateRegistrationForm();
                    }
                    // Aplicar Prism.js a los bloques de código en el modal de credenciales
                    if (node.querySelector('#modal-credenciales')) {
                        Prism.highlightAll();
                    }
                    // Inicializar los inputs de teléfono y labels flotantes
                    setTimeout(() => {
                        initPhoneInputs();
                        initFloatingLabels();
                    }, 100);
